...
val_pipeline = [
    ...
    dict(
        type='Imgaug',
        transforms=[dict(type='GammaContrast', gamma=(0.3, 0.3))]),
    dict(
        type='Normalize',
        mean=[123.675, 116.28, 103.53],
        std=[58.395, 57.12, 57.375],
...
data = dict(
    ...
    train=dict(
        type='VideoDataset',
        ann_file='/root/mv_ca/datasets/train_mm.txt',
        data_prefix='/root/mv_ca/datasets/train',
        pipeline=[
            dict(type='DecordInit'),
            dict(
                type='SampleFrames',
    ...
    val=dict(
        type='VideoDataset',
        ann_file='/root/mv_ca/datasets/validate_mm.txt',
        data_prefix='/root/mv_ca/datasets/validate',
        pipeline=[
            dict(type='DecordInit'),
            dict(
                type='SampleFrames',
...
total_epochs = 58
gpu_ids = [0]
work_dir = './work_dirs/ircsn_dark_dataset'
